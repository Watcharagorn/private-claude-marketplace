...
oldest first** — one deep-analysis agent, an expert review, and automatic implementation of approved
items into the plugin — finishing with a single publish. With a session id: it analyzes only that one
session (both lenses) **interactively** and leaves the ledger/watermark untouched.
...
- **`--headless`** = batch mode, for scheduled/unattended runs (`loom:automate` uses this) — never
  prompts (cap auto-defaults to Newest 12, dead ends stop cleanly), overrides `--review`.
